...
export const tradesSlice = createSlice({
  ...
  reducers: {
    ...
    setHoveredTrade: (state, action) => {
      state.dataStates.hoveredTrade = action.payload;
    },
    setStateTrades: (state, action) => {
      if (state.dataStates.stateTrades.length <= 70) {
        ...
        let tempPartly = [...state.dataStates.stateTradesPartly].filter(
          (trade) => trade.id !== discardedIID
        ); //clearing stateTradesPartly from discarded trade
        ...
        let tempLocationTrades = Object.entries(
          state.dataStates.locationTrades
        ); //clearing locationTrades from discarded trade
        ...
        state.dataStates.locationTrades =
          Object.fromEntries(tempLocationTrades);
      ...
      if (state.dataStates.stateTrades.length > 0) {
        let key =
          state.dataStates.stateTrades[state.dataStates.stateTrades.length - 1][
            givenGroup
          ]; // mkt - fok - sell -buy the givengrouup of the last trade
...
export const loginGetAllProduct = () => async (dispatch) => {
  ...
  try {
    const productsRes = await axios.get(PRODUCT_URL);
